...
if __name__ == '__main__':
    ...
    df = pd.read_csv(filename+'.csv',dtype = {'Data':str,'Transaction Number':str,'UPC Number':str,'Product Name':str,'Customer Account':str,'Qty':float,'Regular Price':float,'Sale Price':float,'Discount':float},header=head,names=['Date','Transaction Number','UPC Number','Product Name','Customer Account','Qty','Regular Price','Sale Price','Discount']) #, parse_dates = ['Date'],infer_datetime_format = False)
